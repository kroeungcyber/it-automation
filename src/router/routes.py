...
session_factory: Any | None = None  # Pre-wired for Plan 2 DB writes; not used in Plan 1

# In-memory store; replaced by DB writes in Plan 2.
# NOT safe across multiple uvicorn workers — run with --workers 1 until Plan 2.
...
async def create_task(request: TaskRequest) -> TaskCreateResponse:
    if classifier is None or queues is None:
        raise HTTPException(status_code=500, detail="Router not initialised — call create_app() first")
    ...
    if scanner.scan(request.raw_input) == ScanResult.SENSITIVE:
        ...
        classification = ClassificationResult(route="local", method="scanner")
    else:
