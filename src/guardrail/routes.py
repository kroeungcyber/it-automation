...
def authorize(
    plan: ActionPlan,
    _: CurrentUser = require_role(Role.IT_ADMIN),
) -> JSONResponse:
    pipeline = get_pipeline()
    result: PipelineResult = pipeline.run(plan)
    log.info("guardrail.authorize.complete", outcome=result.outcome, plan_id=result.action_plan_id)
...
def record_decision(
    ...
    log.info("guardrail.decision.recorded", action_plan_id=action_plan_id, decision=decision.value)
    return {"action_plan_id": action_plan_id, "decision": decision.value}


@router.get("/circuit-breaker/status", status_code=200)
def circuit_breaker_status(_: CurrentUser = require_role(Role.IT_ADMIN)) -> dict:
    cb = get_circuit_breaker()
    states = cb.get_all_states()
    return {
        "states": [
            {"agent_type": s.agent_type, "state": s.state.value, "failure_count": s.failure_count}
            for s in states
        ]
    }
...
def circuit_breaker_reset(
    ...
    log.info("guardrail.circuit_breaker.reset", agent_type=body.agent_type)
